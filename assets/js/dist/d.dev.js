...
btn_next_q.addEventListener('click', function (e) {
  ...
  console.log(answer); //console.log(data.length)
  ...
  if (answer) {
    ...
    if (current_quiz < data.length) {
      ...
      var a = data[current_quiz].answer[0];
      var b = data[current_quiz].answer[1];
      var c = data[current_quiz].answer[2];
      var d = data[current_quiz].answer[3];
...
function fetch_resultant(data, answers) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (answers[i] != data[i].correct) {
      corrections.innerHTML += "\n        <div class=\"question\">\n        <p># ".concat(data[i].question, "</p>\n        <ul class=\"ul__list\">\n            <li class=\"").concat(answers[i] == data[i].answer[0] ? 'incorrect' : '', " ").concat(data[i].correct == data[i].answer[0] ? 'correct' : '', " \">").concat(data[i].a, "</li>\n            <li class=\"").concat(answers[i] == data[i].answer[1] ? 'incorrect' : '', " ").concat(data[i].correct == data[i].answer[1] ? 'correct' : '', "\">").concat(data[i].b, "</li>\n            <li class=\"").concat(answers[i] == data[i].answer[2] ? 'incorrect' : '', " ").concat(data[i].correct == data[i].answer[2] ? 'correct' : '', "\">").concat(data[i].c, "</li>\n            <li class=\"").concat(answers[i] == data[i].answer[3] ? 'incorrect' : '', " ").concat(data[i].correct == data[i].answer[3] ? 'correct' : '', "\">").concat(data[i].d, "</li>\n            <li class=\"exp\">\n                ").concat(data[i].Explication, "\n            </li>\n        </ul>\n    </div>\n        \n        ");
    }
  }
}